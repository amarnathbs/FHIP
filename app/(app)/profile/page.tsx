...
// c336e6b), adapted to current main's server/client conventions (matches
// the established 'use client' + fetch(/api/...) pattern already used by
// every FinancialDataGrid page — see app/(app)/expenses/page.tsx — rather
// than the async-server-component pattern used by read-only pages like
// app/(app)/score/page.tsx; this page needs client-side
// supabase.auth.updateUser() for the email-change flow below, which cannot
// run in a server component). No dedicated Profile page previously existed
// (confirmed absent — no app/(app)/profile/ directory). Backed by the
// EXISTING app/api/user/profile GET/PUT route and profileSchema
// (lib/validation/profile.ts) — this page adds no new profile-fields API
// surface beyond the one genuinely missing column, Contact number
// (migration 0099).
//
// Email changes: email is never read from or written to user_profiles — it
// lives solely in Supabase Auth (auth.users.email) and is changed here via
// supabase.auth.updateUser({ email }), which triggers Supabase's own
...
//
// Date of birth: editing DOB here saves straight through the normal PUT
// /api/user/profile call. Checked every in-scope consumer of
...
// disclosed as a known gap in the final report, not silently glossed over.
type ProfileData = {
