...
// App Review tier-2 fix pass, Fix 4 (Assets — Field Reassessment): Purchase
// Date/Price requested even where meaningless for cash-type accounts.
describe('Asset field metadata (App Review tier-2 Fix 4)', () => {
  ...
  it('no field is ever required (purchase price/date stay optional even where supported — a user may not know the exact figure)', () => {
    for (const key of ['principal_residence', 'motor_vehicle']) {
      const meta = getAssetFieldMetadata(key);
  ...
  it('other grid configs never opt in — every other module keeps its unconditional field list (backward compatible)', () => {
    // Only Assets was in scope for this fix; every other register's fields
    // must remain unconditionally shown, matching pre-fix behaviour exactly.
    const otherConfigs = [
      incomeGridConfig,
      expenseGridConfig,
      liabilityGridConfig,
    ...
    for (const cfg of otherConfigs) expect(cfg.fieldVisibleForRow).toBeUndefined();
  });
});
