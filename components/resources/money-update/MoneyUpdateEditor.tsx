...
// R1.4 Money Update / Money Update Template editor — spec §40-49. Reuses
// R1.3's BlockEditor for the structured narrative sections (pre-populated by
// a Money-Update-specific starter template, spec §43) and the shared
// MetadataSidebar/WorkflowPanel/RevisionHistoryPanel/SaveStatus primitives
// unmodified. Adds Event Date, Affected Audience, and the minimal Source
// picker (spec §49-51).
...
import type { MoneyUpdateEditorPost } from '@/lib/resources/money-update/types';
...
export function MoneyUpdateEditor({
  post: initialPost,
  reference,
  ...
  const [affectedAudience, setAffectedAudience] = useState(initialPost.affected_audience ?? '');
  const [sourceOptions, setSourceOptions] = useState<SourceOption[]>(initialSourceOptions);
  const [sourceIds, setSourceIds] = useState<string[]>(initialPost.sources.map((s) => s.id));
  ...
  const reviewCheck = useMemo(
    ...
    [title, slug, excerpt, meta, blocks, eventDate, initialPost.content_type]
  );

  const isAmberNonRegulatoryWarning = meta.complianceClassification === 'green' && !isTemplate;
  ...
  return (
    <div className="space-y-4 pb-24">
      ...
      <ConfirmDialog
        open={conflict}
        title="This Money Update was updated elsewhere"
        message="Someone else saved changes to this Money Update since you loaded it. Reload the page before saving your changes, or you will lose them."
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_380px]">
        <div className="min-w-0 space-y-4">
          <div className="rounded-card border border-line bg-white p-4">
            ...
            <div className="mt-4">
              <TextAreaField label="Summary / 30-Second Explanation" value={excerpt} onChange={(v) => { setExcerpt(v); markDirty(); }} maxLength={EXCERPT_MAX_LENGTH} rows={3} error={fieldErrors.excerpt || reviewCheck.errors.excerpt} />
            </div>
            {!isTemplate && (
            ...
            {isAmberNonRegulatoryWarning && (
              <p role="note" className="mt-3 rounded-compact border border-attention/30 bg-attention/5 p-2 text-xs text-attention">
                This Money Update is classified GREEN. If this covers tax, regulation, interest rates, government policy or another current-affairs topic, consider whether AMBER (Additional Review Required) is more appropriate.
              </p>
            )}
          </div>

          <div className="rounded-card border border-line bg-white p-4">
            <h2 className="mb-1 text-sm font-semibold text-ink">Structured Sections</h2>
            <p className="mb-3 text-xs text-muted">What Happened, Why It Matters, Who May Be Affected, Financial Health Impact, and How FHIP Can Help — pre-populated below. Additional blocks may be added after the structured core.</p>
          ...
          {!isTemplate && (
            <div className="rounded-card border border-line bg-white p-4">
              <SourcePicker options={sourceOptions} selectedIds={sourceIds} onChangeSelection={(ids) => { setSourceIds(ids); markDirty(); }} onSourceCreated={(s) => setSourceOptions((prev) => [...prev, s])} />
            </div>
          )}
