...
**2026-09-22 (Module 11 AI Remediation R2):** `-e MODULE11_ -e OPENAI_API_KEY` forward Module 11's own real-provider configuration (`MODULE11_AI_PROVIDER`, `MODULE11_AI_MODEL`, `MODULE11_AI_TIMEOUT_MS`, `MODULE11_AI_MAX_TRANSIENT_RETRIES`, `MODULE11_AI_BATCH_MODE`, `MODULE11_SCHEDULER_MAX_HOUSEHOLDS_PER_RUN`) and its credential `OPENAI_API_KEY` (deliberately distinct from `AIE_OPENAI_API_KEY`). Same defect class as every entry above: without these terms the console could say `MODULE11_AI_PROVIDER=openai` while the runtime silently ran the mock. Both default to the mock / fail-closed AUTH when unset. This same edit also brought the quoted line back in step with `amplify.yml`'s `-e STRIPE_ -e RAZORPAY_` terms (added by the 2026-09-21 payments fix but never mirrored here — a drift `tests/unit/m12cServerOnlySecretBoundary.test.ts` was already failing on `main` over).
