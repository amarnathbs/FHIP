...
| `manual` | manual | **false** | **PARTIAL** — `lib/services/investment-intelligence/manualImporter.ts` exists and is wired into the same canonical pipeline (`ii_source_documents` → `ii_accounts` → `ii_instruments` → `ii_transactions` → `ii_holding_snapshots`) with checksum idempotency, but its own header comment states it is "the controlled, deterministic manual/test importer... NOT the production CAS parser" — i.e. it is a fixture-JSON importer proven in tests, not a shipped user-facing manual-entry form. `parser_available` was never flipped for it. **R11-FINAL closure round correction (2026-08-25)**: "fully wired into the same canonical pipeline" was not quite accurate as originally written — a live-DEV test this round found it never actually called the cross-source identity check `documentProcessing.ts` uses, so manual-source evidence arriving AFTER CAMS/KFintech evidence for the same transaction silently duplicated instead of linking (import-order dependency). Fixed this round — see `R11_CROSS_SOURCE_RECONCILIATION.md`'s closure-round addendum and `R11_MANUAL_RECONCILIATION.md` MR13. |
