...
**LIVE-R11-008 (Conflict)**: The identity-resolution LOGIC correctly classifies the conflicting pair as `'conflict'` (hand-verified in `R11_MANUAL_RECONCILIATION.md` MR15, and independently proven via `scripts/r11_rls_certification.mjs`'s fresh PGlite replay of every migration file including `0084`). On live DEV specifically, right now, the conflicting row's INSERT (`status='review_required'`) and its reconciliation case's INSERT (`discrepancy_type='cross_source_conflict'`) both fail — a real, INSERT-based probe this round found migration `0082`'s two CHECK constraint updates are NOT actually live on DEV (an earlier SELECT-based probe in this same round wrongly reported them present; a SELECT filter cannot test a CHECK constraint). See migration `0084` and the final closure report for the exact required action. Graded BLOCKED, not FAIL.
...
1. Apply migration `0083` (`professional_profiles` etc. — entirely new tables) and `0084` (completes `0082`'s constraint updates) to DEV.
