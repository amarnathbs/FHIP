...
1. The manual row's current values are captured into a new `pre_publication_manual_snapshot jsonb` column (current_value, cost_base, institution, owner, investment_type, master_item_key, annual_contribution, risk_profile, captured_at) — the audit-traceable "what it was before" record.
