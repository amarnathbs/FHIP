...
**Residual gap found in the prior closure pass (2026-09-05, earlier the same day): now CLOSED.** That pass found the fix existed only inside this unmerged Wave 6 worktree/branch, and that `origin/main`'s own copy of `scripts/admin_a02_wave2_certification.mjs` (inherited via the Wave 2 merge, commit `6fdcf7e`) still contained the original, unfixed, moving-`origin/main`-ref SECTION 7 — a genuine, still-open residual distinct from what Wave 6 itself reported as closed.

**Closure (this pass, PO-1 recording pass, 2026-09-05 later the same day):** the recommended single-file cherry-pick was performed and landed directly on `origin/main` at commit `b032eb0` ("fix(admin-a02-wave2): pin certification harness baseline to a fixed SHA"), by a separate, isolated change outside this A1 branch's own commits. This branch fetched and rebased onto that commit before this pass began. Independently reconfirmed by this pass: `git show origin/main:scripts/admin_a02_wave2_certification.mjs` now pins `PRE_WAVE2_BASELINE_SHA = '1b40b0be0bbb6b7d67b611e08ca255e68562abf1'` (byte-identical to the Wave 6 worktree's own already-verified copy — diffed directly, zero difference); re-running the harness (in the Wave 6 worktree, whose script is now confirmed byte-identical to `origin/main`'s) reproduces **352 passed, 0 failed, exit 0** fresh. The residual is closed — the corrected harness is proven correct AND now lives on the canonical branch that actually shipped Wave 2.
