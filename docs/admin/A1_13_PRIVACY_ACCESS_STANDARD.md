...
Every future task touching personal/behavioural data (ADM-26, 30-40, 42-46) must, before implementation, pass through: AU Privacy Act/APP review, India DPDP Act 2023/Rules 2025 review (checked against then-current commencement status), collection-notice/purpose-compatibility, retention/deletion design, hosting/cross-border review, minors consideration, and a decision on whether a formal PIA is required. **A1 does not perform any of these reviews** — it records the requirement so no future implementation wave skips it. `REG-14` (audit-evidence retention period) remains explicitly unresolved and requires exactly this kind of jurisdiction review before a number is picked — not decided here, not decided by guessing.
