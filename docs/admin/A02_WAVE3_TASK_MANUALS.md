...
9. **Common errors.** "Could not update source" (network/server error, shown via `alert()`) — no partial state is left; retry is safe.
10. **Recovery.** Re-run the action; it is idempotent (re-approving an already-approved source is a no-op write).
11. **Audit evidence.** `approved_by`/`approved_at` columns on the row itself serve as the audit trail for approval; no separate audit-log row is written for status changes (classified `NOT_AUDITED_NOT_REQUIRED` — see the Audit Matrix; the row-level columns are the authoritative record, matching the pre-existing pattern for this table).
12. **Rollback/reversal.** Suspend → Reinstate reverses a suspension. There is no automated way to return an `approved` source to `draft`; that would require a direct database action (not exposed in Admin by design — approval is meant to be forward-moving except for the suspend/reinstate safety valve).
...
15. **Functionality status.** Operational, connected this Wave (previously `BACKEND_WITHOUT_UI`).
16. **Last certification phase/date.** Wave 3, 2026-09-01.
