...
**Admin A0.2 Wave 4 (Round 2) update, ADM-16 (Manage Related Content) and ADM-17 (Manage Context Mapping) only** — the "remove" step of both tasks previously returned success (200) even when the target relationship/mapping had already been removed or never existed (a bare `.delete()` with no way to distinguish zero matched rows from one). Recovery guidance: removing an item now returns 404 ("This relationship/mapping no longer exists.") if it is already gone — this is expected and safe (nothing was silently skipped; the item was already in the desired end state), not an error requiring investigation. Repeated clicks on a stale "Remove" button (e.g. from a second browser tab) will now show this 404 instead of a second false "removed" confirmation. No other part of either task changed. Every other Resources CRUD task's DELETE-zero-row contract was NOT reviewed or changed this round (a platform-wide convention decision was deliberately deferred to the Product Owner — see the Wave 4 report's PO4-4).

**Admin A0.2 Wave 4 (Round 2) update, ADM-08/ADM-11/ADM-12/ADM-13 (revision-history "Versions" panel only, all four content types)** — the read-only revision-history list (`GET .../[id]/versions`) previously checked only that the caller was signed in, relying entirely on the database's own row-level security to withhold data from a non-Resources-staff caller (which it always correctly did — no draft or unpublished content was ever actually disclosed). It now also independently checks Resources-staff membership at the API layer before querying, consistent with every sibling editor route, and returns 403 ("You don't have permission to view revision history.") for a non-staff authenticated caller instead of a silent empty list. No staff-facing behaviour changes.
