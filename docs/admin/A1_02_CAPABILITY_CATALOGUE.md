...
**Update (A2–A5 Master Execution Programme, A3.3 slice):** this split has been executed — see `docs/admin/A2A5_02A_A3_CAPABILITY_SPLIT_EXECUTION.md`. `requireAdmin()` itself is unchanged and remains the underlying implementation; the 34 call sites that used it directly now call `requireBenchmarksAdmin()` (CAP-16a), `requireRecommendationsAdmin()` (CAP-16b), or `requireAIPlatformAdmin()` (CAP-16c), each a thin wrapper with identical authorization behaviour. Unit-tested for behavioural equivalence; the full live-DEV 9-caller-type matrix this kind of change ordinarily requires remains blocked pending DEV credentials (see that document's §6). CAP-16 itself is retained in this table below as the shared underlying mechanism, not removed.
...
1. **CAP-16 (`requireAdmin`) is a single broad boolean covering 3 unrelated functional domains** (Benchmarks/Recommendations/AI Admin) where Standard §2 requires independent capabilities — see §2 above. Pre-existing, not an A1-introduced defect; flagged for A2/A3. **Split executed, A2A5 A3.3** — see the update note in §2 above and `A2A5_02A_A3_CAPABILITY_SPLIT_EXECUTION.md`.
