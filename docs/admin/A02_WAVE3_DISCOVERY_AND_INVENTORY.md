...
| BWU-3 | `GET /api/admin/resources/glossary/terms` | Zero callers anywhere in the repo (`grep` for `glossary/terms` returns only the route file itself); the route's own header comment states it exists for a **future** Money-Update/FAQ term-linking flow that does not exist yet | **Deferred** (§10) — read-only, harmless, but building the linking UI is new scope beyond this Wave's bounded mandate, and removing a working, documented-intent endpoint on the strength of "no caller yet" is exactly the kind of premature removal the brief's Remove-disposition rules warn against absent a replacement/obsolescence proof |
...
|---|---|---|---|
| PLC-1 | `/admin/resources/analytics` (the Analytics nav destination, visible to Analyst/Resource Admin/Super Admin) | `app/(app)/admin/resources/analytics/page.tsx` — a real, capability-gated route that renders only a title, a one-line "read-only Admin analytics area" description and a neutral "introduced in subsequent authorised waves" statement; asserted by its own Wave 1 test to contain **no digit of any kind**; no control, no export, no telemetry | **Disclosed tension, not unilaterally resolved.** Per this Wave's own disposition rules, a visible destination that completes no task should be "hidden until implemented." But this exact page was deliberately designed and certified in Wave 1 as the intended interim state for an Analyst-only user (whose *only* capability today is `resourceAnalytics` — hiding it would leave that caller with literally no Admin menu at all), it contains no interactive control (so it does not violate the narrower "no clickable fake task" rule literally), and Standard §1.2/this Wave's own "do not reopen certified work absent a direct regression" bar applies. **Recorded in the Deferred/PO Decision register (§10/§Certification report) for an explicit Product Owner call** — hide it now (leaving Analyst-only users with an empty menu) or leave it as Wave 1 designed it — rather than resolved unilaterally in either direction. |

### 5.5 `CONNECTED_INCOMPLETE` register

| # | Surface | Evidence | Disposition |
|---|---|---|---|
| CI-1 | Scheduling (`Schedule` transition on all 4 content types) | Wave 2 made the validation rule **correct and consistent** across all four content types, but confirmed fresh this Wave (§13): `components/resources/editor/WorkflowPanel.tsx` still has **no Schedule action in the UI at all** (re-read directly, unchanged since R1.3/Wave 2); there is still no scheduled-publishing worker anywhere in the repo (`grep -r "scheduled_at" lib/ app/api` outside the validation/RPC files themselves returns nothing resembling a cron/worker); the one already-`status='scheduled'` DB path is reachable only by direct RPC call, not through any button | **Not touched.** Wave 2's own D-3 finding already correctly classified this and explicitly recommended not building a worker without separate authorisation — re-confirmed, not re-litigated. No Schedule control exists to hide (there is nothing clickable to remove), so this is recorded as `CONNECTED_INCOMPLETE`+`Defer` rather than `PLACEHOLDER`+`Hide`, because there is no misleading surface presently visible — the incompleteness is that the *feature* doesn't exist yet, not that a broken control does. |
...
**No route was removed on the strength of this ambiguity.** Per the stop-condition rule ("if an external caller cannot be conclusively resolved, do not remove the route"), every route this investigation touched was either connected (2) or deferred (2), never removed.
