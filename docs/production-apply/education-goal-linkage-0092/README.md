# Production apply package: Education Fund / Children Investment -> Goal Linkage (0092)
...
|---|---|---|---|
| `0092_education_children_investment_goal_linkage.sql` | **NOT applied** (this sandbox has no DDL-execution capability — no psql, no Docker, no SQL-execution RPC; same documented limitation as every prior release's DEV migration step) — fully certified against PGlite instead (32/32, real Postgres 18/WASM engine, real RLS + real trigger) | **NOT applied** — confirmed live: `master_financial_items` still returns `is_active: true` for both `education_fund` and `children_investment`, with migration 0074's original governance_note verbatim (i.e. production has 0072-0074 but not 0092) | Apply `01_0092_education_children_investment_goal_linkage.sql` to DEV first, then to production, in that order |
| Cross-tenant `goal_funding_sources` ownership gap this migration fixes | Confirmed **live and real** on DEV via `scripts/egl_live_dev_security_probe.mjs` — a forged insert (Tenant A's own goal + Tenant B's private investment, submitted under Tenant A's own JWT) returned HTTP 201 (succeeded) against DEV's current pre-0092 schema | Not independently reproduced against production (would require creating throwaway auth users against production, which this agent is not authorized to do) — but production runs the byte-identical `goal_funding_sources` schema/RLS policy from migration 0009 with no later ownership fix, so the same gap is presumed present until 0092 is applied there too | Apply 0092 to close it in both environments |
...
intended — and real catalogue rows for the public one), so 0092 has no
...
3. Run `01_0092_education_children_investment_goal_linkage.sql` in full. It is self-contained (`begin; ... commit;`) and idempotent (every statement is a plain `UPDATE`, `CREATE OR REPLACE FUNCTION`, `DROP POLICY IF EXISTS`/`CREATE POLICY`, or an `INSERT ... SELECT` guarded by `NOT EXISTS`).
...
- **Verified (read-only, publishable-key REST, negative-controlled) against live production:**
  - Current `is_active: true` state of both retired catalogue items (proving 0092 has not yet landed there).
...
- **Verified live against DEV (real Supabase project, real throwaway auth users, real JWTs, real PostgREST calls — not simulated):** `scripts/egl_live_dev_security_probe.mjs` reproduced the actual pre-0092 vulnerability (forged cross-tenant `goal_funding_sources` insert succeeds today) plus a negative control proving the harness itself is sound.
- **NOT performed by the agent — requires human/orchestrator execution:**
  - Actually applying `01_0092...sql` to DEV or to production.
...
- `01_0092_education_children_investment_goal_linkage.sql` — the migration itself (identical to `supabase/migrations/0092_education_children_investment_goal_linkage.sql` on the feature branch).
