...
| **Retirement** | "Import retirement statement" (FDH-12) | FDH-3 lifecycle → generic CSV detection/extraction → activity classification → balance reconciliation → account/member matching → payslip (FDH-9) / bank / rollover reconciliation → `lib/retirement-import-bridge/` (canonical READ) → `lib/import-bridge/adapters/retirementAdapter.ts` + `fdh12_apply_retirement_proposal()` (canonical WRITE) → canonical `retirement_accounts` | **ENGINE + DATABASE EVIDENCE MODEL + BRIDGE + UI ENTRY POINT ALL IMPLEMENTED (FDH-12); LIVE-DEV CERTIFICATION PENDING.** `app/(app)/retirement/page.tsx` + `components/retirement/RetirementStatementImportPanel.tsx` + 7 routes under `app/api/financial-data-hub/retirement-statement/`. Migration `0112` is **NOT APPLIED to DEV or production** — PGlite-certified only (53/53 real-Postgres checks incl. cross-tenant, same-tenant authority, duplicate/concurrent/stale apply, SMSF refusal, and a harness self-check), plus 382 unit tests. Canonical Retirement is a SUMMARY-BALANCE register with no event ledger, so FDH-12 applies a single-row field patch and posts NO statement activity anywhere (`FDH12_REUSE_AND_GAP_AUDIT.md` §0). Only FOUR certified fund-neutral CSV layouts exist — **no named Australian super fund adapter** (`FDH12_AU_SUPER_STATEMENTS.md`), no PDF, no OCR. India ingestion is PARTIAL (EPF passbook CSV only; six canonical India gaps registered, none patched inside FDH-12). SMSF is routed to the existing SMSF module, never imported as ordinary super (`FDH12_SMSF_BOUNDARY.md`). Do not treat this row as live-certified until `0112` is applied and `FDH12_LIVE_DEV_CERTIFICATION.md` is closed. |
