...
**What this pass could and could not do.** This session's Node/TypeScript toolchain could not be reliably installed after several attempts (see `LR_2026_09_21_RECONCILIATION_ADDENDUM.md` §3), so none of the six oracle scripts below could be **re-executed** this pass — the numbers in §1–§8 are **not** freshly re-proven and should be read as 09-14 Tier 4 evidence, not as reconfirmed today. What this pass did instead: (a) read the exact current source for every code path the oracles exercise, to check whether the underlying logic has changed since 09-14, and (b) ran what genuinely could be run without the blocked toolchain — the RLS/cross-tenant tests (native `fetch`, no dependency on `computeDashboard()`), reported in `LR_SECURITY_AND_TENANT_ISOLATION_CERTIFICATION.md`, not here.

**§3, P0-1 (SMSF property loan subtracted twice) — superseded, not simply fixed.** Confirmed via direct commit inspection (not a fresh oracle run): `ea95507` (2026-09-14, same day as this report) fixed the exact scenario documented in this section — Net Worth was −230,000, is now correctly 135,000 for that scenario. `368d98f` (2026-09-17) then found `ea95507`'s fix was too broad (it excluded every SMSF-tagged liability from `totalLiabilities`, not only ones already netted inside a fund's own valuation, so a liability merely tagged `owner='smsf'` with no linked fund vanished from Net Worth entirely — counted zero times instead of exactly once) and reverted `totalLiabilities`/`totalLiabilityMonthlyRepayments`/`liabilityByType` to source from the unfiltered liabilities array again. The fixing engineer's own commit message discloses, in the engineer's own words, that correctly distinguishing "already netted inside `totalRetirement`" from "not netted anywhere" needs either a per-loan/per-fund correlation key that does not exist today, or a gross (not netted) SMSF fund valuation — **flagged back to the Product Owner as an open item**, not silently left broken. A regression test (`smsfHouseholdIsolation.test.ts`) was added reproducing the original double-subtraction, so a future change narrowing these fields again would fail loudly. **This pass did not re-run the fixture in §3 live to get a fresh number** — doing so by hand-deriving the arithmetic from raw table reads (rather than executing the real `computeDashboard()`) was deliberately avoided as a fabrication risk (see the master addendum). Status: **the exact double-subtraction in §3 is fixed; a related, narrower architectural question remains open and needs a Product Owner ruling**, not "FAIL" and not "PASS" without qualification.
- Migration `0148` (2026-09-19) separately fixes an unrelated regression `0137` introduced in `smsf_recompute_fund()` (Detailed-mode holdings edits failing `42501`) — confirmed on `origin/main`. This does not change the Net Worth arithmetic above; it's the P0-2 finding from the companion final report.

**§6, F7 / P2-3 (SMSF contribution leak into household forecast) — re-checked directly, confirmed UNCHANGED.** `grep` for `smsfContributionGuard` in `lib/engines/dashboard.ts` returns zero matches today, same as 09-14 — the guard is still applied only in `forecastData.ts`'s retirement-forecast branch, and `computeDashboard()` (`dashboard.ts:1066`) still sums `input.retirement` unfiltered into `retirementPersonalMonthlyContribution`. **Still open.**

**§8, insurance detection (P2-2) — re-checked directly, confirmed UNCHANGED.** `dashboard.ts:1105-1109` still filters strictly on `i.cover_type === 'income_protection'` with no `master_item_key` fallback (unlike every sibling register, which the code's own comments say was deliberately hardened this way). No `cover_type` field was found added to any insurance form component. **Still open.**

**Not re-checked this pass:** §1 (taxonomy), §2/§4 (cash flow), §5 (loan economics), §7 (goals), P1-4 (Twin vs Dashboard divergence) — carried forward as 09-14 evidence only.
