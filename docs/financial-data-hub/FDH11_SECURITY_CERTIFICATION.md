...
## Not run this pass (disclosed)

Live-DEV security probes against the NEW schema (real anon/authenticated cross-tenant PostgREST requests against `fdh_investment_statements` on the hosted Supabase project) could not be executed — migration `0106` could not be applied to live DEV from this sandbox (no DDL execution mechanism available; see `FDH11_LIVE_DEV_CERTIFICATION.md`). The PGlite certification above exercises the identical RLS/trigger/constraint definitions against real Postgres and is the strongest evidence available this pass.
