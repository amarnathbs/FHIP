...
| Financial Twin / benchmark | income band, expense ratio, housing, remittance, Net Worth | private loader (twinData.ts:137-144, 255-289), no bank data, no superseded flag, raw currency sums | the shared `loadDashboard()` (no private copy) + `buildCanonicalFinancialSnapshot`: combined `housing` group + home-loan debt service, planned remittance, balance-sheet rows in reporting currency, latest 12 snapshots; a failed read fails the run closed | combined | DC-04, DC-13, DC-14, DC-18, GAP-02, EXP-G10 | WP-05 | **switched** |
| Forecast | baseline income and expense, contributions | `loadDashboard`; contributions not converted; null frequency treated as monthly (forecastData.ts:403, 691, 922, 1500-1552) | Net Worth / resilience: shared `loadDashboard()`; retirement / debt / investment / cross-border: the canonical register loaders + rules (`forecastCanonicalInputs.ts`); variance: goals converted per goal, failed read = `unavailable` | combined (via Dashboard) / registers | DC-13, DC-14, DC-18, GAP-RET-02 | WP-05 | **switched** |
| Reports (free + premium) | appendix line items, Net Worth, staleness | reportSnapshotResolver.ts:345-371 (includes superseded rows, no bank, no currency); freshness from manual registers only (:158-166, 203-207) | one snapshot per report: canonical appendix (`reportCanonicalAppendix.ts`), one portfolio total with the unpublished bucket disclosed, "not in Net Worth" items beside Net Worth; staleness and Data Quality include imported data (`lib/read-models/freshness.ts`) | combined | DC-08, DC-09, DC-10, EXP-G11 | WP-06 | **switched** |
...
| `lib/read-models/freshness.ts` (WP-06) | `fdh_transactions` (`approved_at`), `fdh_transaction_allocations`, `fdh_transaction_links` | **Justified.** One-row "newest change" probes for report staleness and Data Quality freshness, kept inside the read models so no service or engine reads an FDH table directly (DC-19). A failure contributes nothing (never artificially fresh or stale). |
