...
- **WP-10 (migration 0208, branch `feature/canonical-wp10-11`):** G3 closed (atomic persist, forward port of the unmerged 0198; one statement per document, unique index), G4 matching quality closed (currency / approval / duplicate / already-matched filters, no blind limit, nearest date first, one bank debit per repayment, candidate ids kept), G5 closed (`computeStatementTotals`), G6 closed (warnings, GST and candidates persisted; never-populated columns are E with user copy).
- **WP-11 (migration 0209, same branch) — MANDATORY GATE:** G1, G2, G4 (Apply re-verification, back-match after bank approval, candidate picker), G7 (Statement history on the Liabilities tab; the grid badges stay with WP-07), G8, G10, G11, G12, G13 and X-01 (liability) closed. The liability ledger registry has no open gap. G9 still needs the consumer switch (WP-03) in the same release.
