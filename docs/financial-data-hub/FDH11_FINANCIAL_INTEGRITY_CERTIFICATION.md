...
Sections 108-113 (live-DEV Buy/Sale/Dividend/Funding/Withdrawal tests against the real running app) could not be executed — migration `0106` is not applied to live DEV (see `FDH11_LIVE_DEV_CERTIFICATION.md`). The pure-function/PGlite evidence above is the certification basis for this pass; it proves the RULES are correctly encoded, not that a real end-to-end upload against hosted Supabase produces the same outcome, which remains to be confirmed once the migration is applied by someone with DDL access.
